...
begin
  GbInstitution := LcPar.Estabelecimento;
  GInstitution.Registro.Codigo              := LcPar.Estabelecimento;
  GInstitution.EntityFiscal.Registro.Codigo := GbInstitution;
  GInstitution.EntityFiscal.getByKey;

  GInstitution.User.Registro.Codigo := LcPar.Usuario;

  GbStock := 1;

  UniApplication.Cookies.SetCookie('_email', email.Text, Date + 1.0); // Expires 1 day from now
  UniApplication.Cookies.SetCookie('_empresa', GInstitution.EntityFiscal.Registro.ApelidoFantasia, Date + 1.0);
  ...
  begin
    ...
    UniApplication.Cookies.SetCookie('_institution_id', IntToStr(GbInstitution), Date + 7.0);
    UniApplication.Cookies.SetCookie('_user_id', IntToStr(GInstitution.User.Registro.codigo), Date + 7.0);
    UniApplication.Cookies.SetCookie('_stock_id', IntToStr(GbStock), Date + 7.0);
  End;
...
begin
  ...
  GInstitution.User.Mailing.Registro.Email := email.Text;
  if not GInstitution.User.Mailing.Valida then
  Begin
    email.setFocus;
    Exit;
  End
