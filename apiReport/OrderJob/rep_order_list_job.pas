...
type
  TRepOrderListJob = class(TRepOrderList)
...
var
  RepOrderListJob: TRepOrderListJob;
...
procedure TRepOrderListJob.DetailBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  with Qry do
  Begin
...
procedure TRepOrderListJob.Ft_CategoryBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TRepOrderListJob.GroupBySql;
begin
  inherited;

end;

procedure TRepOrderListJob.InnerJoinSql;
begin
  inherited;
  InnerJoinTxt := concat(
                    InnerJoinTxt,
                    ' inner join tb_order_job orj ',
                    ' on (ord.id = orj.id) and (ord.tb_institution_id = orj.tb_institution_id) ',
                    ' inner join tb_entity en ',
                    'on (en.id = orj.tb_customer_id)'
                  );
end;

procedure TRepOrderListJob.OrderBySql;
begin
  inherited;
  OrderByTxt := ' order by ord.dt_record, en.name_company ';
end;

procedure TRepOrderListJob.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TRepOrderListJob.SelectSql;
begin
  inherited;
  SelectTxt := concat(
                  SelectTxt,
                  ' , orj.number, en.name_company '
                );

end;

procedure TRepOrderListJob.SetParameters;
begin
  inherited;
...
procedure TRepOrderListJob.setVariable;
begin
  inherited;
  SubValue := 0;
  TotalValue :=0;
...
procedure TRepOrderListJob.WhereSql;
begin
  inherited;
  if ( Par.Entidade > 0) then
  Begin
