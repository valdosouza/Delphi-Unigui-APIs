...
type
  TRepOrderListPurchase = class(TRepOrderList)
...
var
  RepOrderListPurchase: TRepOrderListPurchase;

implementation

{$R *.dfm}

{ TRepOrderListPurchase }

procedure TRepOrderListPurchase.DetailBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  with Qry do
  Begin
...
procedure TRepOrderListPurchase.Ft_CategoryBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TRepOrderListPurchase.GroupBySql;
begin
  inherited;
end;

procedure TRepOrderListPurchase.InnerJoinSql;
begin
  inherited;
  InnerJoinTxt := concat(
                    InnerJoinTxt,
                    ' inner join tb_order_purchase op ',
                    ' on (ord.id = op.id) and (ord.tb_institution_id = op.tb_institution_id) ',
                    ' inner join tb_entity en ',
                    'on (en.id = op.tb_provider_id)'
                  );
end;

procedure TRepOrderListPurchase.OrderBySql;
begin
  inherited;
  OrderByTxt := ' order by ord.dt_record, en.name_company ';
end;

procedure TRepOrderListPurchase.QRBand3BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
...
procedure TRepOrderListPurchase.SelectSql;
begin
  inherited;
  SelectTxt := concat(
                  SelectTxt,
                  ' , op.number, en.name_company '
                );
end;

procedure TRepOrderListPurchase.SetParameters;
begin
  inherited;
...
procedure TRepOrderListPurchase.setVariable;
begin
  inherited;
  SubValue := 0;
  TotalValue :=0;
...
procedure TRepOrderListPurchase.WhereSql;
begin
  inherited;
  if ( Par.Entidade > 0) then
  Begin
