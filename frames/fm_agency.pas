...
type
  TFmAgency = class(TBaseFrameList)
...
begin
  ...
  Begin
    ...
    sql.Clear;
    sql.Add(concat(
               'SELECT en.id, en.name_company, en.nick_trade, a.aliq_com ',
               'FROM tb_entity en ',
               ' inner join tb_agency a ',
               ' on (a.id = en.id) ',
               'where a.tb_institution_id =:tb_institution_id ',
...
procedure TFmAgency.listarSobMovimento;
begin
  cds_search.Active := False;
  with UMM.Qr_Crud do
  Begin
...
procedure TFmAgency.sb_registerClick(Sender: TObject);
begin
  openCadAgency(0,DBLCB_Lista);
end;

procedure TFmAgency.sb_searchClick(Sender: TObject);
begin
  openSeaAgency(DBLCB_Lista);
end;

procedure TFmAgency.setFDataFim(const Value: TDate);
begin
  FDataFim := Value;
end;

procedure TFmAgency.setFDataInicio(const Value: TDate);
begin
  FDataInicio := Value;
end;

procedure TFmAgency.setFPeriodo(const Value: Boolean);
begin
  FPeriodo := Value;
end;

procedure TFmAgency.setFTipoData(const Value: Integer);
begin
  FTipoData := Value;
end;
