...
uses
   SysUtils, Classes, DB, forms,dialogs, windows,Graphics,ExtCtrls,
   jpeg, pngimage, UnFunctions;

type
  TTrataExcecao = class(TThread)
  private
...
constructor TTrataExcecao.Create(CreateSuspended: Boolean;E : Exception);
begin
...
destructor TTrataExcecao.Destroy;
begin

  inherited;
end;

procedure TTrataExcecao.Pc_Erro(Pc_MsgErro: String);
begin
  Pc_LogTarefasDelphi(concat('GESTAOWEB - ',Pc_MsgErro));
end;

procedure TTrataExcecao.execute;
begin
  HandleExcept;
end;

procedure TTrataExcecao.HandleExcept;
var
  Lc_I:integer;
...
Begin
  ...
  Begin
    ...
    winexec('taskkill /f /im RoteadorGestao.exe',sw_hide);
    exit;
  end;
